...
<div class="flex tems-center justify-center w-full">
    <div class="max-w-xl w-full flex flex-col items-center overflow-hidden relative p-8">
      ...
      <img src="../../assets/img/dush-logo.png" class="w-32 my-24" alt="Dush logo">
      <h1 class="font-bold text-3xl text-dush-text ">Log in</h1>
      <h2 class="font-bold mt-2">Or <a href="/auth/register" class="text-dush-green">register</a></h2>

      <form @submit.prevent="submitLogin" class="w-full mt-8">
        ...
        <div class="flex mb-4"  :class="[(usernameType === 'phone') ? 'mr-2': '']">
          <div class="flex items-center" v-if="(usernameType=== 'phone')">
<label for="countryCode" class="sr-only">phone prefix</label>
<input list="browsers" name="countryCode" id="countryCode" class="bg-gray-100 py-4 px-2 w-24 mr-2 rounded-lg focus:outline-none" placeholder="+52">
<datalist id="browsers">
  <option v-for="country in countries" :key="country.name" :value="country.dialCode"></option>
</datalist>
          </div>
          <div
        ...
        <a href="#" class="underline font-semibold">I forgot my pasword</a>
        <button class="w-full text-2xl text-white font-bold bg-dush-darkblue py-4 rounded mt-12">Log in</button>
      </form>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      async submitLogin(){
          ...
          if(this.form.phone_prefix){
              ...
              await this.$axios.$post('api/v1/login', rForm).then(response => {
                if(response.status === 'success'){
                  console.log('succes?')
                  this.$axios.setToken(response.token, 'Bearer')
                 this.$auth.$storage.setCookie('token', response.token, false)
                }
              }).catch(error => console.log(error,'error')).finally(()=>this.clearForm())
